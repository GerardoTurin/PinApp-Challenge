...
public interface UserDeviceJPARepository extends JpaRepository <UserDeviceEntity, Long> {
    ...
    @Query("SELECT ud FROM UserDeviceEntity ud WHERE ud.device.companyId = :companyId AND ud.user.id = :userId")
    List<UserDeviceEntity> findAllByCompanyIdAndByUserId(Long companyId, Long userId);
}
